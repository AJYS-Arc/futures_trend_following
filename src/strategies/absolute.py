...
Absolute trend-following strategy, grouped by sector for allocation.

Within each sector, the strategy goes long every asset with a positive trend
score and short every asset with a negative trend score - position sign is
driven purely by the asset's own score, not by its rank relative to sector
peers. Sector membership still matters for two things: (1) leg weights
(equal or inverse-volatility) are computed across a sector's own signed
names, and (2) sector sleeves are combined using equal weights or equal-risk
contribution (ERC), with an optional overall volatility target. 
"""
...
def _validate_inputs(scores: pd.DataFrame, sectors: pd.Series,
...
def _sector_sleeves_one_date(scores: pd.Series, sectors: pd.Series,
                             vol: pd.Series | None) -> dict[str, pd.Series]:
    """Build one directional sleeve for each sector with at least one signal.

    Every member with a positive score is long, every member with a negative
    score is short
    """
    ...
    for sector in sectors.dropna().unique():
        ...
        sector_scores = scores.reindex(members).dropna()
        sector_scores = sector_scores[sector_scores != 0]

        if sector_scores.empty:
            continue

        signs = np.sign(sector_scores)
        magnitudes = _leg_weights(sector_scores.index, vol)

        sleeve = pd.Series(0.0, index = scores.index, dtype = float)
        sleeve.loc[sector_scores.index] = signs * magnitudes
...
def absolute_trend_weights(scores: pd.DataFrame, sectors: pd.Series,
    ...
    """Build a dates-by-symbol absolute-trend target-weight matrix.
    """
    _validate_inputs(scores, sectors, vol, returns)
    ...
    for date in _rebalance_dates(scores.index, rebalance_frequency):
        ...
        sleeves = _sector_sleeves_one_date(scores.loc[date], sectors, vol_row)
...
if __name__ == "__main__":
    ...
    weights = absolute_trend_weights(scores = trend_scores, sectors = sectors, vol = volatility,
    ...
    print("Net exposure (directional now, not expected to be ~0):", round(float(last.sum()), 6))
